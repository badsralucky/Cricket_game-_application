...
int Game ::takeIntegerInput()
{
    ...
    while (!(cin >> n))
    {
        ...
        cout << "Invalid input! Please try again with the valid input:"
    }
    return n;
}
bool Game :: validateSelectedPlayer(int index){
    int n ;
    vector <Player>palyers;
...
void Game::selectPlayers()
{
    ...
    for (int i = 0; i < playersPerTeam; i++)
    {
        ...
        if (playerIndexTeamA < 0 || playerIndexTeamA > 10)
        {
            ...
            cout << endl <<"Player has benn already selected . Please select other player" << endl;
            goto teamBSelection;
        }
        else
        {
...
void Game::tossChoice(Team tossWinnerTeam){
    cout << "Enter 1 to bat or 2 for Bowl first. " << endl
    <<"1. Bat" << endl 
    <<"2. Bowl" <<endl;
...
void Game::initializePlayers(){
    ...
    cout << bowlingTeam->name << " - " << bowler->name <<" is batting " << endl << endl;
}
void Game::playInnings(){

    for(int i = 0 ; i < maxBalls ; i++){
       cout <<"Please  enter to bowl....." <<endl;
        getchar();
        cout <<"Bowling ....." <<endl;
...
void Game::bat(){
 ...
 if (runsScored != 0) { //if runsScored  = 1 2 3 4 5 6
    cout << endl << bowler->name << " to " << batsman->name << " " << runsScored << "runs!" <<endl <<endl;
    showGameScorecard();

 }else{  //if rubsScored = 0 and the batsman is 'out'
    cout <<endl <<bowler-name << " to " <<batsman->name << "OUT!" << endl <<endl;
    battingTeam->wicketsLost = battingTeam->wicketLost + 1;
    bowler->wicketTaken = bowler->wicketsTaken + 1 ;
    showGameScorecard();
    
    int nextPlayerIndex = battingTeam->wicketsLost;
    batsman = &battingTeam->players[nextPalyerIndex]; 
 }
}
